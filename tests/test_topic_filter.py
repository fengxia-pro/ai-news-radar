...
Traditional ultrasound methods depend predominantly on evidence-based decision trees.
    ...
    def test_slow_professor_august_5_to_20_articles_are_listed_with_verified_metadata(self):
        now = datetime(2026, 8, 20, 22, 32, 49, tzinfo=timezone.utc)
        expected = {
        ...
        payload = build_slow_professor_payload(
            [],
            [],
            generated_at="2026-08-20T22:32:49Z",
            now=now,
        )
        payload_items = {item["url"]: item for item in payload["items"]}
        ...
        self.assertEqual(
            payload_items["https://mp.weixin.qq.com/s/Q2YT4fqDLJ0Y_1rgvb-dSA"]["published_at"],
            "2026-08-20T00:08:13Z",
        )
        self.assertEqual(
        ...
        self.assertIn(
            "把根往下扎一年",
            payload_items["https://mp.weixin.qq.com/s/Q2YT4fqDLJ0Y_1rgvb-dSA"]["summary"],
        )
        self.assertIn(
            "长期积累",
            payload_items["https://mp.weixin.qq.com/s/Q2YT4fqDLJ0Y_1rgvb-dSA"]["article_theme"],
        )
        self.assertIn(
            "关键词学习法",
    ...
    def test_slow_professor_history_is_cumulative_and_cached_items_are_retained(self):
        ...
        self.assertEqual(len(manual_urls), 37)
        ...
        self.assertEqual(payload["total_items"], 38)
        self.assertEqual(payload["recent_7d_count"], 6)
        self.assertEqual(payload["historical_count"], 32)
    ...
    def test_model_scores_include_work_automation_computer_use_and_terminal_use_cards(self):
        ...
        self.assertEqual(len(model_scores["research_metrics"]), 6)
        ...
        self.assertNotIn("五类模型能力", app_js)
    ...
    def test_ai_watts_wechat_article_enters_creator_pool(self):
        now = datetime(2026, 7, 10, 0, 0, tzinfo=timezone.utc)

        class FakeResponse:
            text = """
            <html><head>
              <meta property="og:title" content="可算找到一个能统一管理所有Agent还不用额外订阅的开源项目了！" />
              <meta property="og:description" content="谁说没有原生态的Agent OS的" />
            </head><body></body></html>
            """
        ...
        raw_items = fetch_ai_watts_wechat(FakeSession(), now)
        raw = raw_items[0]
        record = add_source_tier_fields({
            "id": "ai_watts_manual_article",
            "site_id": raw.site_id,
            "site_name": raw.site_name,
            "source": raw.source,
            "title": raw.title,
            "url": raw.url,
            "published_at": "2026-07-09T04:03:23Z",
        ...
        self.assertEqual(raw.site_id, AI_WATTS_WECHAT_SITE_ID)
        self.assertEqual(raw.site_name, "AI沃茨")
        self.assertEqual(raw.title, "可算找到一个能统一管理所有Agent还不用额外订阅的开源项目了！")
        self.assertIn("Agent OS", raw.meta["summary"])
