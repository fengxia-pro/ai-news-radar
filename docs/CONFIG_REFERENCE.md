...
**Secrets(密钥,绝不能写进代码):**
`SOCIALDATA_API_KEY`、`TIKHUB_API_KEY`、`X_BEARER_TOKEN`、`AGENTMAIL_API_KEY`、`AGENTMAIL_INBOX_ID`、`FOLLOW_OPML_B64`、`SLOW_PROFESSOR_WECHAT_FEED_URL`

**Variables(可选,只当急停开关用):**
`SOCIALDATA_ENABLED`、`TIKHUB_ENABLED`、`X_API_ENABLED`、`SLOW_PROFESSOR_WECHAT_ENABLED` —— 不设=默认开;设 `0`=关。

**慢教授公众号专题:** `SLOW_PROFESSOR_WECHAT_FEED_URL` 可填公网 WeWe/RSS 地址；也可以在 `FOLLOW_OPML_B64` 的 OPML 中放入标题为“慢教授的科研江湖”的 WeWe/RSS 源，脚本会优先把它归入慢教授专题，即使 `RSS_MAX_FEEDS` 限制了普通 OPML 抓取数量。专题文件为 `data/latest-slow-professor-3d.json`，按公众号来源和发布时间收录最近 72 小时条目，不做国自然、基金或 AI 关键词过滤。本地 `localhost` WeWe 适合手动生成并提交快照；GitHub Actions 自动更新需要公网 feed。未配置可访问源时，仅显示用户明确给过的微信原文作为“已确认入口”，不使用第三方转载页冒充公众号文章。
